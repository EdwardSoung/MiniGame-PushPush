...
public class UIPanelLobby : UIBase
{
    ...
    {
        if(_curPlayerInfo != GameManager.Instance.GetMainPlayer())
        {
    ...
    {
        _curPlayerInfo = GameManager.Instance.GetMainPlayer();
        _curPlayerPrefab = AddressableManager.Instance.Spawn(GameManager.Instance.GetMainPlayer().UIPrefabName, _characterRoot);
    ...
    {
        EventBus.Instance.Publish(new EventStartStage(GameManager.Instance.GetMainPlayer(), E_STAGE_TYPE.Single_TimeAttack));
    }
}
